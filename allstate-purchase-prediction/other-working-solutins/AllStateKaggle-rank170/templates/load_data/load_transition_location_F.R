...
tmp <- ddply(
  ...
  function(df) {
    ...
    for(i in c(0,1,2,3)) {
      n <- 0
      for(j in c(0,1,2,3)) {
        var.name <- paste("transition_F", i, "vers", j, sep = "_")
        total <- df$total_transition[df$debut_F == i & df$fin_F == j]
        if(length(total) == 0) {
          total <- 1 
        } else {
          total <- total + 1
      ...
      var.total.name <- paste("total_F", i, sep = "_")
    ...
    for(i in c(0,1,2,3)) {
      ...
      for(j in c(0,1,2,3)) {
        var.name <- paste("transition_F", i, "vers", j, sep = "_")
        var.name.prc <- paste("percent_transition_F", i, "vers", j, sep = "_")
...
print("Alimentation table transitions_F")
dbWriteTable(con, "transitions_F", tmp)
